...
function showPaginatedUsers(){
    fetch(`http://localhost:3307/users?page=${currentPage}&limit=${limit}`,{headers:headers()})
       .then(handleErrors)
        .then(data => {
            $("#usesData").html("");
...
function deleteUser(id){
    clearMsgs()
    fetch(`http://localhost:3307/users/${id}`,{method:"DELETE",headers:headers()})
    .then(handleErrors)
    .then(() => {
            showPaginatedUsers()
...
function updateUser(event){
    ...
    fetch(`http://localhost:3307/users/`,
    {method:"PUT",
...
const headers = () =>{
    return {"Content-Type" : "application/json","Authorization":"Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VybmFtZSI6IkVtZWx5LlRveSIsInJvbGUiOiJndWVzdCIsImlhdCI6MTYyMjg0MDE4NywiZXhwIjoxNjIyODQwMjQ3fQ.2rJV5miNKsOwUoHcyik7EgV71RSiROfpUxh5GFTVSBE"}
}
function clearMsgs(){
    $("#pageCard").html("")
}